...
(define (arith-eval expr) (eval (list arith-eval-expr expr)))

(define (test arg expected-result)
  (if (= (arith-eval arg) expected-result)
      #t #f))
 

(test (cons '+ (cons 2 (cons 4 null))) 6)
(test (cons '- (cons 2 (cons 8 null))) -6)
(test (cons '/ (cons 1 (cons 2 null))) (/ 1 2))
(test (cons '* (cons 4 (cons 4 null))) 16)
(test (cons '+ (cons (cons '+ (cons 12 (cons 13 null))) (cons (cons '* (cons 2 (cons (cons '- (cons 15 (cons 7 null))) null))) null))) 41)
(test (cons '/ (cons (cons '* (cons 3 (cons 7 null))) (cons (cons '- (cons 4 (cons (cons '- (cons 11 (cons 8 null))) null))) null))) 21)

;;Współpraca z Wojciech Goska.