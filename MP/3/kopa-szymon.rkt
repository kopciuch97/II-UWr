...
;;; solve ax+by=1
;;; The idea is to let a=bq+r and solve bx+ry=1 
...
(define (make-signed-message sign message) (cons sign message))
(define (message m) (cdr m))
(define (signature s) (car s))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ((enc-msg
    ...
    (make-signed-message (RSA-transform (compress enc-msg) sender-private-key) enc-msg)))


(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (cond [(= (compress (message cyphertext)) (RSA-transform (signature cyphertext) sender-public-key))
         (RSA-decrypt (message cyphertext) recipient-private-key)]
        [else (error "Niepoprawny podpis")]))
