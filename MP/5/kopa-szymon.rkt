...
(define (eq-var x y)
  (cond [(or (empty? x)
        ...
        [(var=? (car x)
                (car y)) (car x)]
        [(var<? (car x)
                (car y)) (eq-var (cdr x) y)]
        [else (eq-var x (cdr y))]))

(define (clause-trivial? c)
   (if [null? (eq-var (res-clause-pos c) (res-clause-neg c))]
       #f
       #t
      ))

(define (resolve c1 c2)
  (let ((var-pos-in-1 (eq-var (res-clause-pos c1)     ;;positive in c1
                           (res-clause-neg c2)))
        (var-pos-in-2 (eq-var (res-clause-pos c2)     ;;positive in c2
                           (res-clause-neg c1)))) 
    (cond
      [(and (null? var-pos-in-1)
            (null? var-pos-in-2)) #f]                      
      [(null? var-pos-in-1)                          
       (res-clause (remove-duplicates-vars (merge-vars (res-clause-pos c1) (remove var-pos-in-2 (res-clause-pos c2))))
                   (remove-duplicates-vars (merge-vars (remove var-pos-in-2 (res-clause-neg c1)) (res-clause-neg c2)))
                   (proof-res var-pos-in-2 (res-clause-proof c2) (res-clause-proof c1)))] 
       
      [else
       (res-clause (remove-duplicates-vars (merge-vars (remove var-pos-in-1 (res-clause-pos c1)) (res-clause-pos c2)))
                   (remove-duplicates-vars (merge-vars (res-clause-neg c1) (remove var-pos-in-1 (res-clause-neg c2))))
                   (proof-res var-pos-in-1 (res-clause-proof c1) (res-clause-proof c2)))]
...
(print "Test clause trivial #t")
(newline)
(define clause1 (res-clause '(p q r) '(q) 'proof))
(clause-trivial? clause1)
(newline)
(print "Test clause trivial #f")
(newline)
(define clause2 (res-clause '(p q r) '(j) 'proof))
(clause-trivial? clause2)
(newline)

(define c1 (res-clause '(p q) '(r) 'proof))
(define c2 (res-clause '(z x) '(q) 'proof))
(resolve c1 c2)
